...
// Remove preloader when page is fully loaded (all assets)
...
// Fallback: Remove after 5 seconds maximum (in case of slow loading or errors)
setTimeout(function () {
  removePreloader();
}, 5000);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (mobilecollapsever) {
    mobilecollapsever.addEventListener("click", function () {
      ...
      // Check if desktop (width > 1024px) - toggle minimenu
      // Otherwise mobile - toggle overlay
      if (window.innerWidth > 1024) {
        // Desktop behavior - toggle minimenu class
        if (document.body.classList.contains("minimenu")) {
          document.body.classList.remove("minimenu");
          // Expand submenus
          var elem = document.querySelectorAll(
