...
class PaySlipController extends Controller
{
    ...
    public function editEmployee($paySlip)
    {
        ...
        // Get allowance and deduction options from employee's payslip type
        $allowanceOptions = [];
        $deductionOptions = [];

        if ($employee && $employee->salary_type) {
            $payslipType = \App\Models\PayslipType::find($employee->salary_type);
            if ($payslipType) {
        ...
        return view('payslip.salaryEdit', compact('payslip', 'employee', 'allowanceOptions', 'deductionOptions'));
    }
    ...
    public function applyComponents(Request $request)
    {
        try {
            ...
            if (!$employee || !$payslip) {
                return response()->json([
                    'success' => false,
                    'message' => __('Employee or payslip not found.')
                ]);
            }

            // Get the payslip type and its allowance/deduction options
            $payslipType = \App\Models\PayslipType::find($employee->salary_type);

            if (!$payslipType) {
                return response()->json([
                    'success' => false,
                    'message' => __('Payslip type not configured for this employee.')
                ]);
            }
            ...
            if ($allowanceOptions->isEmpty() && $deductionOptions->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => __('No allowance or deduction options configured in this payslip type. Please add them in Payslip Type settings first.')
                ]);
            }

            // Get existing components
            $existingAllowances = Allowance::where('employee_id', $employee->id)->pluck('allowance_option')->toArray();
            $existingDeductions = SaturationDeduction::where('employee_id', $employee->id)->pluck('deduction_option')->toArray();

            $appliedCount = 0;
            $details = [];

            // Apply allowances (Earnings & Benefits)
            ...
            foreach ($deductionOptions as $option) {
                if (!in_array($option->id, $existingDeductions)) {
                    ...
                    $deduction->save();
                    $appliedCount++;
                    $details[] = $option->name . ' (Deduction)';
                }
            }

            // Update payslip with ALL components (not just allowances and deductions)
            ...
            if ($appliedCount > 0) {
                $message = __('Successfully applied :count salary components:', ['count' => $appliedCount]) . ' ' . implode(', ', $details);
                return response()->json([
                    'success' => true,
                    'message' => $message
                ]);
                ...
                return response()->json([
                    'success' => true,
                    'message' => __('All salary components from your payslip type are already applied.')
                ]);
            }
