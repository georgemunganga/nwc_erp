...
class SaturationDeductionController extends Controller
{
    public function saturationdeductionCreate($id)
    {
        ...
        $tax_slabs         = $this->taxSlabsForPayrollType(optional($employee)->salary_type);
        return view('saturationdeduction.create', compact('employee', 'deduction_options','saturationdeduc','tax_slabs'));
    }
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create saturation deduction'))
        {
            ...
            $saturationdeduction->tax_slab_id      = $request->tax_slab_id;
    ...
    public function edit($saturationdeduction)
    {
        ...
        if(\Auth::user()->can('edit saturation deduction'))
        {
            if($saturationdeduction->created_by == \Auth::user()->creatorId())
            {
                $deduction_options = DeductionOption::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
                $saturationdeduc = SaturationDeduction::$saturationDeductiontype;
                $tax_slabs = $this->taxSlabsForPayrollType(optional($saturationdeduction->employee)->salary_type);
                return view('saturationdeduction.edit', compact('saturationdeduction', 'deduction_options','saturationdeduc','tax_slabs'));
            }
            else
            {
    ...
    protected function taxSlabsForPayrollType($salaryTypeId)
    {
        return TaxSlab::where('created_by', \Auth::user()->creatorId())
            ->where(function ($query) use ($salaryTypeId) {
                $query->whereNull('payslip_type_id');
                if ($salaryTypeId) {
                    $query->orWhere('payslip_type_id', '=', $salaryTypeId);
                }
            })
            ->get();
    }
