...
class BankAccountController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create bank account'))
        {
            ...
            // Conditional validation based on is_company_account
            if ($request->has('is_company_account') && $request->is_company_account == 1) {
                $rules['payment_name'] = 'required';
            } else {
                $rules['account_type'] = 'required|in:customer,vendor,shareholder,employee';
            }
            ...
            // Check payment name uniqueness only for company accounts
            if ($request->has('is_company_account') && $request->is_company_account == 1) {
            ...
            $account->is_company_account = $request->has('is_company_account') ? 1 : 0;
    ...
    public function update(Request $request, BankAccount $bankAccount)
    {
        if(\Auth::user()->can('create bank account'))
        {
            ...
            $bankAccount->is_company_account = $request->has('is_company_account') ? 1 : 0;
