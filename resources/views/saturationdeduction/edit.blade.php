{{Form::model($saturationdeduction,array('route' => array('saturationdeduction.update', $saturationdeduction->id), 'method' => 'PUT' , 'class'=>'needs-validation', 'novalidate')) }}
    <div class="modal-body">
        <div class="alert alert-light border mb-3">
            <strong>{{__('Employee')}}:</strong> {{ $saturationdeduction->employee->name ?? '-' }}
            @if($saturationdeduction->employee)
                <span class="text-muted ms-2">({{ \Auth::user()->employeeIdFormat($saturationdeduction->employee->employee_id) }})</span>
            @endif
        </div>
    ...
    <div class="card-body p-0">
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    {{ Form::label('tax_slab_id', __('Tax Slab'), ['class' => 'form-label']) }}
                    {{ Form::select('tax_slab_id', $tax_slabs->pluck('name','id')->prepend(__('Select Tax Slab'), ''), null, ['class' => 'form-control select']) }}
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
