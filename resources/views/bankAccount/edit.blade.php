...
<div class="modal-body">
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <div class="form-check form-switch">
                    <input type="checkbox" class="form-check-input" id="is_company_account" name="is_company_account" value="1" {{ $bankAccount->is_company_account ? 'checked' : '' }} onchange="toggleAccountType()">
                    <label class="form-check-label" for="is_company_account">{{ __('Yes') }}</label>
                </div>
            </div>
        </div>
        <div class="col-md-6" id="account_type_wrapper" style="display: {{ $bankAccount->is_company_account ? 'none' : 'block' }};">
            <div class="form-group">
                {{ Form::label('account_type', __('Account Type'), ['class' => 'form-label']) }}<x-required></x-required>
                <select name="account_type" id="account_type" class="form-control" {{ !$bankAccount->is_company_account ? 'required' : '' }}>
                    <option value="">{{ __('Select Account Type') }}</option>
                    <option value="customer" {{ $bankAccount->account_type == 'customer' ? 'selected' : '' }}>{{ __('Customer') }}</option>
                    <option value="vendor" {{ $bankAccount->account_type == 'vendor' ? 'selected' : '' }}>{{ __('Vendor') }}</option>
                    <option value="shareholder" {{ $bankAccount->account_type == 'shareholder' ? 'selected' : '' }}>{{ __('Shareholder') }}</option>
                    <option value="employee" {{ $bankAccount->account_type == 'employee' ? 'selected' : '' }}>{{ __('Employee') }}</option>
                </select>
            </div>
        </div>
        <div class="form-group col-md-6">
        ...
        <div class="col-md-6" id="payment_gateway_wrapper" style="display: {{ $bankAccount->is_company_account ? 'block' : 'none' }};">
            <div class="form-group">
                {{ Form::label('payment_name', __('Payment Gateway'), ['class' => 'form-label']) }}<x-required></x-required>
                <select name="payment_name" id="payment_name" class="form-control" {{ $bankAccount->is_company_account ? 'required' : '' }}>
