                                ...
                                <table class="table datatable">
                                    ...
                                    <tbody class="font-style">
                                        @foreach ($paysliptypes as $paysliptype)
                                            <tr>
                                                <td>
                                                    <div class="fw-bold">{{ $paysliptype->name }}</div>
                                                    <div class="mt-2">
                                                        <div class="text-muted fs-6">{{ __('Tax Slab') }}</div>
                                                        @if($paysliptype->taxSlabs->isEmpty())
                                                            <span class="text-muted fs-7">{{ __('Not assigned yet') }}</span>
                                                        @else
                                                            <div class="d-flex flex-wrap gap-1">
                                                                @foreach($paysliptype->taxSlabs as $slab)
                                                                    <span class="badge bg-light border text-dark">{{ $slab->name }}</span>
                                                                @endforeach
                                                            </div>
                                                        @endif
                                                    </div>
